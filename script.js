    ...
    if(new_password.value!= '' && confirm_password.value != '')
    {
        if(new_password.value.length >= 8 && confirm_password.value.length>=8)
        {
            if(new_password.value ==confirm_password.value){
                ...
                message.innerHTML='<i class="fa-solid fa-circle-check"></i>Both password matched.';
                message.style.color='green';
            }
            else{
                container.style.height='440px';
                message.style.display='block';
                message.innerHTML='<i class="fa-solid fa-triangle-exclamation"></i>Both password not matched.';
